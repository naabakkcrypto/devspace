...
function serverInstructions(config: ServerConfig): string {
  ...
  if (config.toolMode === "codex") {
    const localAgentScope = config.subagents
      ...
      : "";
    return `Use DevSpace for coding work. Call ${toolNames.openWorkspace} once for each project folder or isolated worktree, then keep using its workspaceId. During continued work in the same project or worktree, do not call ${toolNames.openWorkspace} again. Open another workspace only when changing projects, switching checkout/worktree mode, creating another isolated worktree, or when the current workspaceId is rejected. Choose the inspection strategy that maximizes completeness, correctness, and verifiability. Use ${toolNames.read} for directly named files and exec_command for searches, multi-file inspection, tests, builds, and other commands. Use as many read or exec_command calls as needed. Commands use the host's native shell. On Windows, exec_command runs through cmd.exe; invoke powershell.exe explicitly for PowerShell syntax. Never pass PowerShell-only syntax such as 2>$null directly to cmd.exe. Group related inspections only when this preserves complete relevant evidence and clear file provenance. Split the work whenever output size, ambiguity, truncation risk, or independent verification makes separate calls safer. Never omit relevant evidence merely to reduce visible tool calls, output volume, or token use. Use apply_patch for all file modifications and write_stdin to poll or interact with running processes. Follow instructions returned by ${toolNames.openWorkspace}; read applicable instruction and skill files before working in their scope.${localAgentScope}${artifactInstruction}${showChangesInstruction}`;
  }
