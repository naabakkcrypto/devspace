...
function serverInstructions(config: ServerConfig): string {
  ...
  if (config.toolMode === "codex") {
    const localAgentScope = config.subagents
      ? " Native Codex collaboration child-model rules do not govern DevSpace local agent profiles. For DevSpace subagent work, prefer the configured Codex Sol profile and its strongest configured reasoning effort; profile metadata remains requested rather than runtime-verified unless native evidence says otherwise."
      : "";
    return `Use DevSpace for coding work. Call ${toolNames.openWorkspace} once for each project folder or isolated worktree, then keep using its workspaceId. During continued work in the same project or worktree, do not call ${toolNames.openWorkspace} again. Open another workspace only when changing projects, switching checkout/worktree mode, creating another isolated worktree, or when the current workspaceId is rejected. Use the fewest tool calls that preserve correctness, completeness, and verifiability. Keep one active user objective per turn and execute only its necessary subtasks. Broad authorization removes repeated confirmation gates but does not authorize starting later backlog or TODO items. Use ${toolNames.read} for directly named files and exec_command for searches, multi-file inspection, tests, builds, and other commands. Commands use the host's native shell. On Windows, exec_command runs through cmd.exe; invoke powershell.exe explicitly for PowerShell syntax. Never pass PowerShell-only syntax such as 2>$null directly to cmd.exe. Group related inspections when this preserves complete relevant evidence and clear file provenance. Split the work whenever output size, ambiguity, truncation risk, or independent verification makes separate calls safer. After roughly 30 direct tool calls for the same objective, perform a soft efficiency checkpoint: restate the unmet acceptance criteria, consolidate the next calls, and continue only when completion is near, stopping would leave an unsafe or incoherent state, or one causal proof is still missing. Repeated calls that produce no new evidence are diminishing returns; change strategy or stop and report. Once the acceptance criteria are met and proportionate verification is complete, stop and report instead of starting another objective. Never omit relevant evidence solely to reduce visible tool calls, output volume, or token use. Use apply_patch for all file modifications and write_stdin to poll or interact with running processes. Follow instructions returned by ${toolNames.openWorkspace}; read applicable instruction and skill files before working in their scope.${localAgentScope}${artifactInstruction}${showChangesInstruction}`;
  }
...
function registerCodexProcessTools(
  ...
  registerAppTool(
    ...
    "exec_command",
    {
      ...
      description:
        "Run a command in a workspace. Returns its result when it exits during the yield window, otherwise returns a sessionId for write_stdin. Commands use the host's native shell. On Windows, this runs through cmd.exe; invoke powershell.exe explicitly for PowerShell syntax and never pass PowerShell-only syntax such as 2>$null directly to cmd.exe. Prefer grouped commands over equivalent micro-calls when this preserves complete relevant evidence and clear file provenance. Use multiple commands whenever output size, ambiguity, truncation risk, or independent verification makes separate calls safer. Repeated commands without new evidence are diminishing returns; change strategy or stop. Never omit relevant evidence solely to reduce call count, output volume, or token use. Use this for searches, multi-file inspection, tests, builds, package scripts, and long-running processes.",
      inputSchema: {
...
export function createMcpServer(
  ...
  registerAppTool(
    ...
    toolNames.read,
    {
      ...
      description:
        [
          "Read a directly named file in a workspace. Prefer the smallest set of reads that preserves complete relevant context. For multi-file work, combine read and exec_command according to evidence needs. Split the work whenever output size, ambiguity, truncation risk, or independent verification makes separate calls safer. Repeated reads without new evidence are diminishing returns; change strategy or stop. Never omit relevant evidence solely to reduce call count, output volume, or token use.",
